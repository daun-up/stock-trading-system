...
- [x] 잔고 부족 시 명확한 예외와 API 응답 처리 추가
- [x] 매수 주문 금액 계산 규칙 보강
...
- [x] `src/test` 디렉터리 구성
- [x] `AccountBalance` 잔고 예약 단위 테스트
- [x] `OrderService` 중복 주문 요청 테스트
- [ ] 잔고 부족 주문 실패 테스트
- [x] 주문 접수 시 outbox 이벤트 생성 테스트
- [ ] 주문 API validation 테스트

### 3. 예외 처리와 API 응답 정리

- [x] 공통 예외 응답 구조 추가
- [x] validation 실패 응답 형식 통일
- [x] 도메인 예외 타입 추가
- [x] 400/404/409 등 HTTP status 기준 정리
